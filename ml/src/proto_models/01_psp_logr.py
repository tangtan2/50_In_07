...
def get_psp_data_per_player(spark_session: pyspark.sql.SparkSession, player: int) -> pyspark.sql.DataFrame:
    ...
    query = f"""
    (SELECT player_id, winner, d.game_id, team_id_for, event, goals_home AS goals_for, \
    ...
    WHERE "HoA" = 'home' \
    UNION \
    SELECT player_id, winner, d.game_id, team_id_for, event, goals_away AS goals_for, \
...
def create_proto_model(psp_data: pyspark.sql.DataFrame, dump_folder: str, date: str, eval_log, player: int):
    ...
    lr = LogisticRegression(featuresCol='features',
                            labelCol='label')
    param = ParamGridBuilder().addGrid(lr.elasticNetParam, [0, 0.5, 1.0])\
        .addGrid(lr.fitIntercept, [False, True])\
        .addGrid(lr.regParam, [0.2, 0.01]).build()
